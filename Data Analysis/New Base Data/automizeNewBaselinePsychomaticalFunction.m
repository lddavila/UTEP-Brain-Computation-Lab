function automizeNewBaselinePsychomaticalFunction
    function [T] = createMap()
        query = "SELECT referencetime, subjectid, mazenumber FROM live_table WHERE health = 'N/A' OR health = 'none' or health LIKE 'Baseline%';";
...
%                         counter = counter+1;
                    ...
                    %uploads the psychomatical function to the "psychomaticalFunctions"
                    %table in the database
                    sqlwrite(conn,"newBaseLineRotationPointsPsychometricFunctions",data)
                    %                 disp("the following query was written to the travelpixeltable")
                    %                 disp(query)
                    secondaryCounter = secondaryCounter +1;
                catch ME
            ...
            try
                ...
                while secondaryCounter < width(animalsOnThatDate)
                    try
                        ...
                        %uploads the psychomatical function to the "psychomaticalFunctions"
                        %table in the database
                        sqlwrite(conn,"newBaseLineStoppingptsPsychometricFunctions",data)
                        %                 disp("the following query was written to the travelpixeltable")
                        %                 disp(query)
                        secondaryCounter = secondaryCounter +1;
                    catch
            ...
            try
                ...
                while secondaryCounter <= width(animalsOnThatDate)
                    try
                        ...
                        %uploads the psychomatical function to the "psychomaticalFunctions"
                        %table in the database
                        sqlwrite(conn,"newBaseLineTravelPixelPsychometricFunctions",data)
                        %                 disp("the following query was written to the travelpixeltable")
                        %                 disp(query)
                        secondaryCounter = secondaryCounter +1;
                    catch
