function automizereactiontime1stPsychomaticalFunction
    ...
    function [T] = createMap()
        query = "SELECT referencetime, subjectid, mazenumber FROM live_table WHERE LOWER(health) = 'none' OR LOWER(health) ='n/a';";
        ...
        while counter <= size(T,1)
            ...
            while secondaryCounter <= width(animalsOnThatDate)
                try
                    ...
                    query = strcat("SELECT id,referencetime,subjectid,reactiontime1st FROM featuretable WHERE " + ...
                    ...
                    sqlwrite(conn,"basePsychometricFunctions",data)
