function automizePsychomaticalFunction
    ...
    function [xcoordinates,ycoordinates] = createPsychomaticalFunction(searchResults)
        ...
        feeder1Percentage = str2double(regexprep(string(local{1,19}),'%','')) / 100;
        feeder2Percentage = str2double(regexprep(string(local{1,20}),'%',''))/100;
        feeder3Percentage = str2double(regexprep(string(local{1,21}),'%',''))/100;
        feeder4Percentage = str2double(regexprep(string(local{1,22}),'%',''))/100;
    ...
    try
        ...
        while secondaryCounter <= width(animalsOnThatDate)
            try
                ...
                query = strcat("SELECT * FROM live_table WHERE date = '",string(T{counter,1}),"' AND subjectid = '", animalsOnThatDate{secondaryCounter},"';");
                ...
                sqlwrite(conn,"psychomaticalFunctions",data)
                secondaryCounter = secondaryCounter +1;
            catch
