...
%             display(gof3.rsquare)
            ...
            if gof3.rsquare >= .6
                saveas(fighandle3,strcat(dynamicName,"3 Parameter Sigmoid\",string(results{i,1})," ", strrep(string(results{i,2}),"/","-"),".fig"))
                save(strcat(dynamicName,'Sigmoid Data\',string(results{i,1})," ", strrep(string(results{i,2}),"/","-"),'.mat'),'fitobject3') 
            elseif gof4.rsquare >= .6
                saveas(fighandle4,strcat(dynamicName,"4 Parameter Sigmoid\",string(results{i,1})," ", strrep(string(results{i,2}),"/","-"),".fig"))
                save(strcat(dynamicName,'Sigmoid Data\',string(results{i,1})," ", strrep(string(results{i,2}),"/","-"),'.mat'),'fitobject4') 
            elseif gof2.rsquare >= .6
                saveas(fighandle2,strcat(dynamicName,"2 Parameter Sigmoid\",string(results{i,1})," ", strrep(string(results{i,2}),"/","-"),".fig"))
                save(strcat(dynamicName,'Sigmoid Data\',string(results{i,1})," ", strrep(string(results{i,2}),"/","-"),'.mat'),'fitobject2')
            elseif gof1.rsquare > gof5.rsquare
                saveas(fighandle1,strcat(dynamicName,"Lines\",string(results{i,1})," ", strrep(string(results{i,2}),"/","-"),".fig"))
                save(strcat(dynamicName,'Line Data\',string(results{i,1})," ", strrep(string(results{i,2}),"/","-"),'.mat'),'fitobject1')
            elseif gof5.rsquare > gof1.rsquare
                saveas(fighandle5,strcat(dynamicName,"Parabolas\",string(results{i,1})," ", strrep(string(results{i,2}),"/","-"),".fig"))
                save(strcat(dynamicName,'Parabola Data\',string(results{i,1})," ", strrep(string(results{i,2}),"/","-"),'.mat'),'fitobject5')
            end
            if gof3.rsquare < 0
...
query = "SELECT subjectid,date,x1,x2,x3,x4,y1,y2,y3,y4 FROM psychomaticalfunctions;";
