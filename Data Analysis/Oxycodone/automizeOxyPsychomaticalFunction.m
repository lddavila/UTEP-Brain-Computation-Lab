function automizeOxyPsychomaticalFunction
    function [T] = createMap()
        query = "SELECT referencetime, subjectid, mazenumber FROM live_table WHERE LOWER(health) LIKE LOWER('%oxy%');";
...
%                     disp(animalsOnThatDate{secondaryCounter})
                    try
                        ...
                        sqlwrite(conn,"oxyRewardChoicePsychometricFunctions",data)
...
%                     display(theformattedDate)
                    ...
                    %uploads the psychomatical function to the "psychomaticalFunctions"
                    %table in the database
                    sqlwrite(conn,"oxyReactionTimePsychometricFunctions",data)
...
%                         counter = counter+1;
                    ...
                    %uploads the psychomatical function to the "psychomaticalFunctions"
                    %table in the database
                    sqlwrite(conn,"oxyRotationPointsPsychometricFunctions",data)
                    %                 disp("the following query was written to the travelpixeltable")
                    %                 disp(query)
                    secondaryCounter = secondaryCounter +1;
                catch ME
            ...
            try
                ...
                while secondaryCounter < width(animalsOnThatDate)
                    try
                        ...
                        %uploads the psychomatical function to the "psychomaticalFunctions"
                        %table in the database
                        sqlwrite(conn,"oxyStoppingptsPsychometricFunctions",data)
                        %                 disp("the following query was written to the travelpixeltable")
                        %                 disp(query)
                        secondaryCounter = secondaryCounter +1;
                    catch
...
%                         display(animalsOnThatDate{secondaryCounter})
                        ...
                        %uploads the psychomatical function to the "psychomaticalFunctions"
                        %table in the database
                        sqlwrite(conn,"oxyTravelPixelPsychometricFunctions",data)
                        %                 disp("the following query was written to the travelpixeltable")
                        %                 disp(query)
                        secondaryCounter = secondaryCounter +1;
                    catch
