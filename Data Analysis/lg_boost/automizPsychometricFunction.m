function automizPsychometricFunction
    function [T] = createMap()
        query = "SELECT referencetime, subjectid, mazenumber FROM live_table WHERE genotype='lg_boost';";
    ...
    function [theFormattedDate] = formatTheDate(theDate)
        ...
        theFormattedDate=strcat(theMonth,"/",theDay,"/",theYear);
    end
    ...
    function[xcoordinates,ycoordinates] = createStoppingPointsPsychometricFunction(searchResults)
        local = searchResults;
        local2 = [str2double(local.stoptimemethod6)];
        if sum(isnan(local2)) > 0
            %display(local)
            %             display(isnan(table2array(local)))
            %             disp("NAN DETECTED")
        ...
        while i <= numberOfTotalRows
            ...
            %because of atanu's code I have to multiply the stopping points
            %value by 10 to get an accurate reading
            switch feederValue
                %FIX THIS RIGHT HERE ADD THE TRAVEL PIXEL OR OTEHER
                %FEATURE VALUES
                case '1'
                    feeder1Totals = feeder1Totals + (str2double(string(local{i,4}))*10);
                case '2'
                    feeder2Totals = feeder2Totals + (str2double(string(local{i,4}))*10);
                case '3'
                    feeder3Totals = feeder3Totals+(str2double(string(local{i,4}))*10);
                case '4'
                    feeder4Totals = feeder4Totals+(str2double(string(local{i,4}))*10);
            end
    ...
    function rewardChoiceLoop(T,tableToWriteIn)
...
%                     disp(animalsOnThatDate{secondaryCounter})
                    try
                        ...
                        sqlwrite(conn,tableToWriteIn,data)
    ...
    function reactionTimeLoop(T, tableToWriteIn)
        counter = 1;
        while counter <= size(T,1)
            %     try
            animalsOnThatDate = (split(string(T{counter,2}),","));
...
%                     display(theformattedDate)
    ...
    %rotation Points is done
    function rotationPointsLoop(T,tableToWriteIn)
        counter = 1;
        while counter <= size(T,1)
                try
            animalsOnThatDate = (split(string(T{counter,2}),","));
...
%                     display(theDate)
                    theFormattedDate = formatTheDate(theDate);
                    %display(theformattedDate)
                    query = strcat("SELECT id,referencetime,subjectid,rotationptsmethod1 FROM featuretable2 WHERE referencetime ='",theFormattedDate,"'"," AND subjectid = '", animalsOnThatDate{secondaryCounter},"';");
                    disp(animalsOnThatDate{secondaryCounter})
...
%                         counter = counter+1;
    ...
    %stopping points loop is done
    function stoppingPointsLoop(T,tableToWriteIn)
        counter = 1;
        while counter <= size(T,1)
            try
                animalsOnThatDate = (split(string(T{counter,2}),","));
                ...
                while secondaryCounter < width(animalsOnThatDate)
                    try
                        ...
                        theFormattedDate = formatTheDate(theDate);
                        display(theFormattedDate)
                        query = strcat("SELECT id,referencetime,subjectid,stoptimemethod6 FROM featuretable2 WHERE referencetime LIKE '",theFormattedDate,"%'"," AND subjectid = '", animalsOnThatDate{secondaryCounter},"';");

                        disp(query);

                        searchResults = fetch(conn,query);
                        %                 display(searchResults)
                        searchResults2 = rmmissing(searchResults, 'DataVariables',"stoptimemethod6");
                        %display(searchResults)
                        if height(searchResults) == 0
                            %                     display(searchResults)
                            %                     disp("The following query resulted in NaNs")
    ...
    function travelPixelLoop(T,tableToWriteIn)
        counter = 1;
        while counter <= size(T,1)
            try
                animalsOnThatDate = (split(string(T{counter,2}),","));
                ...
                while secondaryCounter <= width(animalsOnThatDate)
                    try
                        ...
                        theFormattedDate = formatTheDate(theDate);
                        %display(theformattedDate)
                        query = strcat("SELECT id,referencetime,subjectid,distanceaftertoneuntillimitingtimestamp FROM featuretable2 WHERE referencetime LIKE '",theFormattedDate,"%'"," AND subjectid = '", animalsOnThatDate{secondaryCounter},"';");
...
% reactionTimeLoop(T,"lg_boost_reactionTimePsychometricFunctions")
%reaction time has been eliminated 

travelPixelLoop(T,"lg_boost_travelPixelPsychometricFunctions")
% %new travel pixel column: distanceaftertoneuntillimitingtimestamp
% 
stoppingPointsLoop(T,"lg_boost_stoppingPointsPsychometricFunctions")
% %new stopping points column: stoptimemethod6
% %the valued in stoptimemethod6 must be mulitplied by 10 to get the actual
% %number of stopping points
% 
% 
rotationPointsLoop(T,"lg_boost_rotationPointsPsychometricFunctions")
% %mew rotation points column: rotationptsmethod1

rewardChoiceLoop(T,"lg_boost_rewardChoicePsychometricFunctions")
end